...
function searchFAQs(query, detailName) {
  ...
  if (!query.trim()) {
    ...
    // Reinitialize FAQ toggle when showing original list
    initializeFAQToggle();
    return;
  }
  ...
  // Improved search with partial matching and case insensitivity
  ...
  faqs.forEach(faq => {
    ...
    // Check if query appears in either question or answer
  ...
  // Initialize toggle for search results
  initializeFAQToggle();
}
...
function clearSearchResults() {
  ...
  // Reinitialize FAQ toggle when clearing search
  initializeFAQToggle();
}
...
@media (min-width: 600px) {
  .chat-support-header h2 { font-size: 1.2rem; }
...
@media (min-width: 768px) {
  ...
  .chat-support-header h2 { font-size: 1.3rem; }
  .back-button i { font-size: 1.2rem; }
}

@media (min-width: 992px) {
  .chat-support-header h2 { font-size: 1.4rem; }
}
